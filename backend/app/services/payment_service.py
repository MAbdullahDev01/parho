...
def _booking_with_rate(booking_id: str, student_clerk_id: str) -> dict:
    try:
        ...
        booking_response = supabase.table("bookings").select("id,student_clerk_id,tutor_clerk_id,status,booking_type,start_at,end_at").eq("id", booking_id).maybe_single().execute()
        ...
        tutor_response = supabase.table("tutor_profiles").select("hourly_rate").eq("clerk_id", booking["tutor_clerk_id"]).maybe_single().execute()
        tutor = tutor_response.data
...
async def create_booking_payment(student_clerk_id: str, booking_id: str, success_url: str, cancel_url: str) -> dict:
    ...
    try:
        ...
        checkout = await safepay.create_checkout(
            amount_pkr=booking["amount_pkr"],
            booking_id=booking_id,
            success_url=success_url,
            cancel_url=cancel_url,
        )
        response = get_supabase().table("payment_intents").insert({
            "booking_id": booking_id,
            "student_clerk_id": booking["student_clerk_id"],
            "tutor_clerk_id": booking["tutor_clerk_id"],
            "provider": "safepay",
            "provider_tracker": checkout["tracker"],
            "amount": float(booking["amount_pkr"]),
            "currency": "PKR",
            "status": "pending",
