...
class SafepayService:
    ...
    async def create_checkout(
        ...
        *,
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                session_response = await client.post(
                    f"{self.base_url}/order/payments/v3/",
                ...
                token_response = await client.post(
                    f"{self.base_url}/client/passport/v1/token",
                    headers={"Authorization": f"Bearer {self.secret_key}", "Content-Type": "application/json"},
                )
        ...
        checkout_url = (
            f"{self.base_url}/checkout?tracker={tracker}"
            f"&tbt={auth_token}&environment={self.environment}&source=hosted"
            f"&redirect_url={httpx.URL(success_url)}&cancel_url={httpx.URL(cancel_url)}"
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                response = await client.get(
                    f"{self.base_url}/reporter/api/v1/payments/{tracker}",
    ...
    def verify_webhook(self, raw_body: bytes, signature: str | None, timestamp: str | None) -> bool:
        ...
        # Safepay's webhook signature format may include a prefix; accept only the
        # documented value or a raw hex digest, never a loosely parsed token.
        candidates = {expected, f"v1={expected}", f"sha256={expected}"}
