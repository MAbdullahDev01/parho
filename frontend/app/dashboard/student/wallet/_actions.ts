...
export async function getStudentWallet() {
  ...
  const request = backend('/api/backend/wallet');
...
export async function holdBookingPayment(bookingId: string, amount: number) {
  const id = await userId();
  const request = backend(`/api/backend/wallet/bookings/${encodeURIComponent(bookingId)}/hold?amount=${encodeURIComponent(amount)}`);
  ...
  return await response.json() as WalletTransaction;
}

export async function releaseBookingPayment(bookingId: string) {
  const id = await userId();
  const request = backend(`/api/backend/wallet/bookings/${encodeURIComponent(bookingId)}/release`);
  const response = await fetch(request.url, { method: 'POST', headers: { ...request.headers, 'x-user-id': id } });
