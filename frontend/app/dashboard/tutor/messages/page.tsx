...
export default async function TutorMessagesPage({ searchParams }: { searchParams: Promise<{ bookingId?: string }> }) {
  ...
  if (!bookingId) {
    return (
      <div>
        <Link href="/dashboard/tutor/bookings" className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-ink"><ArrowLeft className="h-3.5 w-3.5" /> Back to bookings</Link>
