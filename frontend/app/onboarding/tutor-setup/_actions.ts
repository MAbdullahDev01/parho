...
export async function getTutorSubjects(): Promise<ActionResult<string[]>> {
  ...
  try {
    ...
    if (!Array.isArray(subjects)) {
      return {
        success: false,
        error: "The server returned an invalid subject list.",
      };
    }
...
export async function uploadTutorTranscript(
  ...
  try {
    ...
    if (!(file instanceof File)) {
      return {
        success: false,
        error: "Please select a transcript file.",
      };
    }
    ...
    const response = await fetch(
      getBackendUrl("/api/backend/tutors/transcripts"),
      {
        method: "POST",
    ...
    if (!body.transcript) {
      return {
        success: false,
        error: "The server did not return the uploaded transcript.",
      };
    }
...
export async function saveTutorProfile(
  ...
  try {
    ...
    /*
     * Enforce the gating rule server-side as well.
     * The UI also enforces this before submission.
     */
    if (
      ...
      return {
        success: false,
        error:
          "An O-Level Cambridge transcript only permits O-Level teaching.",
      };
    }

    if (
      !payload.transcripts.some(
        (transcript) => transcript.transcript_type === "cambridge",
      )
    ) {
      return {
        success: false,
        error: "A Cambridge transcript is required.",
      };
    }

    const response = await fetch(
      getBackendUrl("/api/backend/tutors/profile"),
      {
        method: "POST",
        headers: {
    ...
    const savedProfile =
      (await response.json()) as SavedTutorProfile;
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Unable to save your tutor profile.",
    };
  }
}